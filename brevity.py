...
async def on_message(message) -> None:
    ...
    await m_channel.send('Cool {}! I will send you the abbreviation of the last {} messages in private.'.format(author_global_name, num_messages))
    ...
    await collect_messages_and_build_prompt(m_channel, prompt_body_file_path, num_messages)
    ...
    prompt = greeting_sentence + '{}\n\n'.format(prompt_header) + prompt_body + end_sentence
    ...
    if len(brevity_response) > MAXIMUM_DISCORD_MESSAGE_LENGTH:
        ...
        for i in range(0, len(brevity_response), MAXIMUM_DISCORD_MESSAGE_LENGTH):
            await m_author.send(brevity_response[i:i+MAXIMUM_DISCORD_MESSAGE_LENGTH])
    else:
        await m_author.send(brevity_response)



async def collect_messages_and_build_prompt(m_channel: discord.TextChannel, prompt_body_file_path, num_messages: int = None):
    if not num_messages:
        num_messages = MAXIMUM_MESSAGES_COLLECTION_LENGTH

    m_channel_history = m_channel.history(limit=int(num_messages))

    def write_prompt_to_file(collected_msgs: list, prompt_body_file_path) -> None:
        with open(prompt_body_file_path, 'a') as file:
            for collected_msg in collected_msgs:
                ...
                if collected_msg.reference and collected_msg.reference.resolved:
                    ...
                    file.write('{}" (responds to {}): {}\n'.format(collected_msg_author.name, referenced_msg_author.name, collected_msg.content))
                    continue
                
                file.write('{}: {}\n'.format(collected_msg_author.name, collected_msg.content))
